...
export const C = {
  ...
  econ: '#2A6B5C',
  econSoft: 'rgba(42,107,92,0.12)',
  stamp: '#A03D30',
  stampSoft: 'rgba(160,61,48,0.10)',
  ledger: '#33567E',
  ledgerSoft: 'rgba(51,86,126,0.12)',
  brass: '#8E7228',
  brassSoft: 'rgba(142,114,40,0.16)',
  night: '#252C28',
  white: '#F9FBF7',
  ghost: 'rgba(32,40,36,0.30)',
} as const;
...
export const Chip = ({children, tone = 'white', style}: {children: ReactNode; tone?: 'white' | 'ink' | 'econ' | 'stamp' | 'ledger' | 'brass' | 'night'; style?: CSSProperties}) => {
  const bg = tone === 'white' ? C.white : C[tone];
  const color = tone === 'white' || tone === 'brass' ? C.ink : C.white;
  const border = tone === 'white' ? `2px solid ${C.ink}` : '2px solid rgba(32,40,36,0.25)';
...
export const LabelBlock = ({children, color = C.econ, ink = false, size = 28, style}: {children: ReactNode; color?: string; ink?: boolean; size?: number; style?: CSSProperties}) => (
...
export const Neg = ({children, size = 24, style}: {children: ReactNode; size?: number; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, color: C.stamp, fontSize: size, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>
    <span style={{display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: size + 6, height: size + 6, borderRadius: 999, border: `3px solid ${C.stamp}`, color: C.stamp, fontSize: size - 4, fontWeight: 950, flexShrink: 0}}>×</span>
    <span>{children}</span>
  </span>
);

export const FileStamp = ({children, delay = 0, tone = 'stamp'}: {children: ReactNode; delay?: number; tone?: 'stamp' | 'econ' | 'ledger'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const color = tone === 'stamp' ? C.stamp : tone === 'econ' ? C.econ : C.ledger;
  return <span style={{display: 'inline-flex', padding: '3px 12px', border: `3px double ${color}`, color, fontSize: 22, fontWeight: 950, rotate: '-2deg', opacity: p, scale: 0.9 + p * 0.1, whiteSpace: 'nowrap', backgroundColor: 'rgba(249,251,247,0.65)'}}>{children}</span>;
};

export const FileTitle = ({children}: {children: ReactNode}) => (
...
export const Mark = ({children, color = C.econ}: {children: ReactNode; color?: string}) => (
...
export const IconChip = ({icon, tone, title, children}: {icon: ReactNode; tone: string; title: string; children: ReactNode}) => (
  <div style={{display: 'flex', alignItems: 'center', gap: 12, backgroundColor: C.filingDeep, borderLeft: `6px solid ${tone}`, padding: '8px 12px'}}>
...
export const Shell = ({code, kicker, title, children}: {code: string; kicker: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.filing, color: C.ink, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', left: 62, top: 44, padding: '10px 16px', backgroundColor: C.night, borderLeft: `8px solid ${C.econ}`}}>
      <span style={{fontSize: 22, fontWeight: 950, color: C.white, letterSpacing: 2}}>第21讲 · {code}</span>
    </div>
    <div style={{position: 'absolute', left: 250, right: 72, top: 34, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 22, borderBottom: `3px solid ${C.ink}`, paddingBottom: 10}}>
      ...
      <span style={{fontSize: 20, fontWeight: 850, color: C.econ, textAlign: 'right', whiteSpace: 'nowrap'}}>{kicker}</span>
    </div>
