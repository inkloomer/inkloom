...
export const Chip = ({children, tone = 'chalk', style}: {children: ReactNode; tone?: 'chalk' | 'warm' | 'cool' | 'ink' | 'prep' | 'flash' | 'green'; style?: CSSProperties}) => {
...
export const LabelBlock = ({children, color = C.flash, ink = false, size = 28, style}: {children: ReactNode; color?: string; ink?: boolean; size?: number; style?: CSSProperties}) => (
...
export const Neg = ({children, dark = false, size = 26}: {children: ReactNode; dark?: boolean; size?: number}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, color: dark ? C.danger : C.negDark, fontSize: size, fontWeight: 900, whiteSpace: 'nowrap'}}>
    <span style={{display: 'inline-flex', fontSize: size + 4, fontWeight: 950, transform: 'rotate(0deg)'}}>×</span>
    <span>{children}</span>
  </span>
);

export const Stamp = ({children, delay = 0, tone = 'flash'}: {children: ReactNode; delay?: number; tone?: 'flash' | 'prep' | 'chalk' | 'green'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const bg = tone === 'flash' ? C.flash : tone === 'prep' ? C.prep : tone === 'green' ? C.green : 'transparent';
  const color = tone === 'chalk' ? C.chalk : C.white;
  return <span style={{display: 'inline-flex', padding: '3px 12px', border: `3px solid ${tone === 'chalk' ? C.chalk : tone === 'prep' ? C.prep : tone === 'green' ? C.green : C.flash}`, backgroundColor: bg, color, fontSize: 22, fontWeight: 950, rotate: '-2deg', opacity: p, scale: 0.9 + p * 0.1, whiteSpace: 'nowrap'}}>{children}</span>;
};

export const GhostNumeral = ({n}: {n: string}) => <span style={{fontSize: 40, fontWeight: 950, color: C.ghost, width: 34, lineHeight: 1}}>{n}</span>;
