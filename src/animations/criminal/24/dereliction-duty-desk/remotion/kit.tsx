...
export const C = {
  ...
  deskDeep: '#DFDCD1',
  ink: '#24221C',
  inkSoft: 'rgba(36,34,28,0.76)',
  seal: '#9E3B28',
  sealSoft: 'rgba(158,59,40,0.10)',
  steel: '#3C5A72',
  steelSoft: 'rgba(60,90,114,0.12)',
  bronze: '#8C742A',
  bronzeSoft: 'rgba(140,116,42,0.16)',
  night: '#2A2822',
  white: '#FAF9F2',
  ghost: 'rgba(36,34,28,0.30)',
  amber: '#A8842C',
  ward: '#4A6B82',
  pulse: '#B04A32',
} as const;
  ...
  return (
    <div
      style={{
        ...
        opacity: 0.55,
        ...(vertical ? {width: thickness, height: '100%', scale: `1 ${p}`, transformOrigin: 'center top'} : {height: thickness, scale: `${p} 1`, transformOrigin: 'left center'}),
      }}
    />
  );
};
...
export const Chip = ({children, tone = 'white', style}: {children: ReactNode; tone?: 'white' | 'ink' | 'steel' | 'seal' | 'bronze' | 'night'; style?: CSSProperties}) => {
  const bg = tone === 'white' ? C.white : C[tone];
  const color = tone === 'white' || tone === 'bronze' ? C.ink : C.white;
  const border = tone === 'white' ? `2px solid ${C.ink}` : '2px solid rgba(36,34,28,0.25)';
...
export const SoftHi = ({children, style}: {children: ReactNode; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, backgroundColor: C.bronzeSoft, padding: '4px 12px', borderRadius: 4, color: C.ink, fontSize: 24, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>{children}</span>
);
...
export const OfficialStamp = ({children, delay = 0, tone = 'seal'}: {children: ReactNode; delay?: number; tone?: 'seal' | 'steel' | 'bronze'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const color = tone === 'seal' ? C.seal : tone === 'steel' ? C.steel : C.bronze;
  return <span style={{display: 'inline-flex', padding: '3px 12px', border: `3px double ${color}`, color, fontSize: 22, fontWeight: 950, rotate: '-2deg', opacity: p, scale: 0.9 + p * 0.1, whiteSpace: 'nowrap', backgroundColor: 'rgba(250,249,242,0.65)'}}>{children}</span>;
};
...
export const IconChip = ({icon, tone, title, children}: {icon: ReactNode; tone: string; title: string; children: ReactNode}) => (
  <div style={{display: 'flex', alignItems: 'center', gap: 12, backgroundColor: C.deskDeep, borderLeft: `10px solid ${tone}`, padding: '9px 12px'}}>
...
export const Shell = ({code, kicker, title, children}: {code: string; kicker: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.desk, color: C.ink, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', left: 62, top: 44, padding: '10px 16px', backgroundColor: C.night, borderLeft: `8px solid ${C.seal}`}}>
      <span style={{fontSize: 22, fontWeight: 950, color: C.white, letterSpacing: 2}}>第24讲 · {code}</span>
    </div>
