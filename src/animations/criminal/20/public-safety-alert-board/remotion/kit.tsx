...
export const C = {
  ...
  siren: '#B03328',
  sirenSoft: 'rgba(176,51,40,0.10)',
  night: '#2B3440',
  nightSoft: 'rgba(43,52,64,0.12)',
  yellow: '#A8842E',
  yellowSoft: 'rgba(168,132,46,0.16)',
  green: '#3E7C5A',
  greenSoft: 'rgba(62,124,90,0.12)',
  white: '#FAFAF4',
  ghost: 'rgba(37,35,28,0.30)',
} as const;
...
export const Chip = ({children, tone = 'white', style}: {children: ReactNode; tone?: 'white' | 'ink' | 'siren' | 'night' | 'yellow' | 'green'; style?: CSSProperties}) => {
  const bg = tone === 'white' ? C.white : C[tone];
  const color = tone === 'white' || tone === 'yellow' ? C.ink : C.white;
  const border = tone === 'white' ? `2px solid ${C.ink}` : '2px solid rgba(37,35,28,0.25)';
...
export const LabelBlock = ({children, color = C.siren, ink = false, size = 28, style}: {children: ReactNode; color?: string; ink?: boolean; size?: number; style?: CSSProperties}) => (
...
export const SoftHi = ({children, style}: {children: ReactNode; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, backgroundColor: C.yellowSoft, padding: '4px 12px', borderRadius: 4, color: C.ink, fontSize: 24, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>{children}</span>
);
...
export const Neg = ({children, size = 24, style}: {children: ReactNode; size?: number; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, color: C.siren, fontSize: size, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>
    <span style={{display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: size + 6, height: size + 6, borderRadius: 999, border: `3px solid ${C.siren}`, color: C.siren, fontSize: size - 4, fontWeight: 950, flexShrink: 0}}>×</span>
    <span>{children}</span>
  </span>
);

export const SirenStamp = ({children, delay = 0, tone = 'siren'}: {children: ReactNode; delay?: number; tone?: 'siren' | 'green' | 'yellow'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const color = tone === 'siren' ? C.siren : tone === 'green' ? C.green : C.yellow;
...
export const Shell = ({code, kicker, title, children}: {code: string; kicker: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.board, color: C.ink, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', left: 62, top: 44, padding: '10px 16px', backgroundColor: C.night, borderLeft: `8px solid ${C.siren}`}}>
      <span style={{fontSize: 22, fontWeight: 950, color: C.white, letterSpacing: 2}}>第20讲 · {code}</span>
    </div>
    <div style={{position: 'absolute', left: 250, right: 72, top: 34, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 22, borderBottom: `3px solid ${C.ink}`, paddingBottom: 10}}>
      ...
      <span style={{fontSize: 20, fontWeight: 850, color: C.siren, textAlign: 'right', whiteSpace: 'nowrap'}}>{kicker}</span>
    </div>
