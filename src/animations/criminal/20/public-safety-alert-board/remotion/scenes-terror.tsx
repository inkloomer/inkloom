...
export const TerrorAlertWardScene = () => {
  ...
  return (
    <Shell code="05" kicker="第四节 · 恐怖型犯罪" title="恐怖型犯罪：从组队到动手的递进线">
      ...
      >
        <Enter delay={3} marker="timeline-rail-board" style={{position: 'absolute', left: 0, top: 0, width: 1776, height: 210}}>
          <Panel tone={C.siren} watermark={<Siren size={140} color={C.siren} strokeWidth={1.6} />} style={{height: '100%', padding: '12px 18px'}}>
            ...
            <div style={{position: 'relative', height: 110, marginTop: 8}}>
              <div style={{position: 'absolute', left: 10, top: 56, width: 1300, height: 5, backgroundColor: C.siren, opacity: 0.55}} />
              <div style={{position: 'absolute', left: 1316, top: 46, width: 0, height: 0, borderTop: '12px solid transparent', borderBottom: '12px solid transparent', borderLeft: `20px solid ${C.siren}`}} />
              {stages.map((stage, index) => (
                <div key={index} style={{position: 'absolute', left: stage.left, top: 8, width: 380}}>
                  ...
                  <div style={{fontSize: 22, fontWeight: 950, color: stage.tone, marginTop: 4}}>{stage.title}</div>
                  <div style={{fontSize: 19, fontWeight: 850, color: C.inkSoft}}>{stage.sub}</div>
                  <div style={{width: 4, height: 20, backgroundColor: stage.tone, marginLeft: 22, marginTop: 2}} />
                </div>
              ))}
              <div style={{position: 'absolute', left: dotX, top: 40, width: 40, height: 40, borderRadius: 20, backgroundColor: C.siren, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: dotIn, boxShadow: '0 3px 9px rgba(37,35,28,0.4)'}}>
                <Siren size={22} color={C.white} strokeWidth={2.4} />
              </div>
              <div style={{position: 'absolute', left: dotX - 60, top: 84, fontSize: 16, fontWeight: 900, color: C.siren, opacity: dotIn, whiteSpace: 'nowrap'}}>打击时点逐段前移</div>
        ...
        <Enter delay={40} marker="stage-org-board" style={{position: 'absolute', left: 0, top: 234, width: 566, height: 510}}>
          <Panel tone={C.siren} watermark={<Users size={150} color={C.siren} strokeWidth={1.6} />} style={{height: '100%', padding: '12px 16px'}}>
            ...
            <div style={{position: 'relative', height: 400, marginTop: 10}}>
          ...
          <Panel tone={C.yellow} watermark={<ShieldCheck size={150} color={C.yellow} strokeWidth={1.6} />} style={{height: '100%', padding: '12px 16px'}}>
            ...
            <div style={{position: 'relative', height: 400, marginTop: 10}}>
              ...
              <div style={{position: 'absolute', left: 0, top: 228, width: 520, border: `3px solid ${C.lock}`, padding: '9px 12px', fontSize: 20, fontWeight: 880, lineHeight: 1.45}}>
                <b style={{color: C.lock}}>既遂＝资助被接收：</b>招募的人员被恐怖组织接收→<Mark color={C.lock}>既遂</Mark>；同类：资助危害国家安全犯罪活动罪（第107条）
              </div>
            </div>
          </Panel>
        </Enter>

        <Enter delay={128} marker="stage-prepare-board" style={{position: 'absolute', left: 1180, top: 234, width: 596, height: 510}}>
