...
export const C = {
  ...
  chartDeep: '#E0E6DD',
  ink: '#1F2823',
  inkSoft: 'rgba(31,40,35,0.76)',
  ward: '#3E7C6F',
  wardSoft: 'rgba(62,124,111,0.12)',
  pulse: '#B23A32',
  pulseSoft: 'rgba(178,58,50,0.10)',
  chart2: '#3E6478',
  chart2Soft: 'rgba(62,100,120,0.12)',
  amber: '#A8842E',
  amberSoft: 'rgba(168,132,46,0.16)',
  night: '#28322C',
  white: '#FAFDF8',
  ghost: 'rgba(31,40,35,0.30)',
} as const;
...
export const Chip = ({children, tone = 'white', style}: {children: ReactNode; tone?: 'white' | 'ink' | 'ward' | 'pulse' | 'chart2' | 'amber' | 'night'; style?: CSSProperties}) => {
  const bg = tone === 'white' ? C.white : C[tone];
  const color = tone === 'white' || tone === 'amber' ? C.ink : C.white;
  const border = tone === 'white' ? `2px solid ${C.ink}` : '2px solid rgba(31,40,35,0.25)';
...
export const LabelBlock = ({children, color = C.ward, ink = false, size = 28, style}: {children: ReactNode; color?: string; ink?: boolean; size?: number; style?: CSSProperties}) => (
...
export const SoftHi = ({children, style}: {children: ReactNode; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, backgroundColor: C.amberSoft, padding: '4px 12px', borderRadius: 4, color: C.ink, fontSize: 24, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>{children}</span>
);
...
export const Neg = ({children, size = 24, style}: {children: ReactNode; size?: number; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, color: C.pulse, fontSize: size, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>
    <span style={{display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: size + 6, height: size + 6, borderRadius: 999, border: `3px solid ${C.pulse}`, color: C.pulse, fontSize: size - 4, fontWeight: 950, flexShrink: 0}}>×</span>
    <span>{children}</span>
  </span>
);

export const StampMark = ({children, delay = 0, tone = 'pulse'}: {children: ReactNode; delay?: number; tone?: 'pulse' | 'ward' | 'amber'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const color = tone === 'pulse' ? C.pulse : tone === 'ward' ? C.ward : C.amber;
  return <span style={{display: 'inline-flex', padding: '3px 12px', border: `3px double ${color}`, color, fontSize: 22, fontWeight: 950, rotate: '-2deg', opacity: p, scale: 0.9 + p * 0.1, whiteSpace: 'nowrap', backgroundColor: 'rgba(250,253,248,0.65)'}}>{children}</span>;
};

export const WardTitle = ({children}: {children: ReactNode}) => (
...
export const Mark = ({children, color = C.ward}: {children: ReactNode; color?: string}) => (
...
export const IconChip = ({icon, tone, title, children}: {icon: ReactNode; tone: string; title: string; children: ReactNode}) => (
  <div style={{display: 'flex', alignItems: 'center', gap: 12, backgroundColor: C.chartDeep, borderLeft: `6px solid ${tone}`, padding: '8px 12px'}}>
...
export const Shell = ({code, kicker, title, children}: {code: string; kicker: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.chart, color: C.ink, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', left: 62, top: 44, padding: '10px 16px', backgroundColor: C.night, borderLeft: `8px solid ${C.pulse}`}}>
      <span style={{fontSize: 22, fontWeight: 950, color: C.white, letterSpacing: 2}}>第18讲 · {code}</span>
    </div>
    <div style={{position: 'absolute', left: 250, right: 72, top: 34, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 22, borderBottom: `3px solid ${C.ink}`, paddingBottom: 10}}>
      ...
      <span style={{fontSize: 20, fontWeight: 850, color: C.ward, textAlign: 'right', whiteSpace: 'nowrap'}}>{kicker}</span>
    </div>
