...
const Chip=({children,color=C.mint}:{children:ReactNode;color?:string})=><span style={{display:'inline-block',padding:'7px 13px',backgroundColor:color,color:C.bg,fontFamily:'Consolas, monospace',fontSize:21,fontWeight:900}}>{children}</span>;
...
const Canvas=({code,title,children}:{code:string;title:string;children:ReactNode})=><AbsoluteFill style={{overflow:'hidden',backgroundColor:C.bg,color:C.white,fontFamily:'"Segoe UI", "Microsoft YaHei", sans-serif'}}>
  ...
  <div style={{position:'absolute',left:64,top:42,fontFamily:'Consolas, monospace',fontSize:18,fontWeight:800,color:C.mint}}>ACCOUNT SIGNAL / {code}</div>
  <h1 style={{position:'absolute',left:64,top:74,margin:0,fontSize:56,lineHeight:1.05,fontWeight:850}}>{title}</h1>
  <div style={{position:'absolute',left:64,right:64,top:158,height:3,backgroundColor:C.grid}}/>{children}
  <div style={{position:'absolute',left:64,right:64,bottom:28,display:'flex',justifyContent:'space-between',fontFamily:'Consolas, monospace',fontSize:17,color:C.muted}}><span>CRIMINAL / TOPIC 19</span><span>TRACE WHO CONTROLS THE FUNDS</span></div>
</AbsoluteFill>;
...
export const LawfulFundsScene=()=>{const f=useCurrentFrame();const steps=[{x:70,icon:LockKeyhole,title:'掐卡',result:'盗窃既遂',color:C.coral},{x:620,icon:ArrowRightLeft,title:'转账',result:'不再定罪',color:C.mint},{x:1170,icon:Banknote,title:'取款',result:'另看银行',color:C.cyan}];return <Canvas code="02" title="合法资金：事实占有说沿行为顺序定性">
  <div data-layout="three-packet-terminal-chain" data-visual-anchor="flow-path" data-text-treatments="label-block,thin-underline,stamp" data-visual-grammar="sequence,absorption,secondary-victim" data-focal-rule="card-lock-completes-theft-before-later-transfer" data-focal-channels="icon,motion,connector,annotation" style={{position:'absolute',inset:'190px 70px 72px'}}>
    {steps.map((s,i)=><Enter key={s.title} delay={8+i*34} style={{position:'absolute',left:s.x,top:170,width:440,height:390,border:`4px solid ${s.color}`,backgroundColor:C.panel,padding:38}}>{i===0?<LockKeyhole size={82} color={s.color}/>:null}{i===1?<ArrowRightLeft size={82} color={s.color}/>:null}{i===2?<Banknote size={82} color={s.color}/>:null}<div style={{fontFamily:'Consolas,monospace',fontSize:19,color:C.muted,marginTop:32}}>PACKET 0{i+1}</div><div style={{fontSize:48,fontWeight:900,marginTop:8}}>{s.title}</div><div style={{fontSize:29,fontWeight:850,marginTop:34,color:s.color}}>{s.result}</div><div style={{fontSize:23,color:C.muted,marginTop:18,lineHeight:1.45}}>{['乙失去控制，甲建立排他控制','左口袋转到右口袋，被前行为吸收','柜台多数说诈骗；ATM 多数说盗窃'][i]}</div></Enter>)}
