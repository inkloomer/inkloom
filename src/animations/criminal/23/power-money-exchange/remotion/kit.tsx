...
export const C = {
  ...
  scrollDeep: '#E3D8BC',
  ink: '#26221A',
  inkSoft: 'rgba(38,34,26,0.76)',
  indigo: '#3A4A6B',
  indigoSoft: 'rgba(58,74,107,0.12)',
  rouge: '#A03A30',
  rougeSoft: 'rgba(160,58,48,0.10)',
  patina: '#4E6E5D',
  patinaSoft: 'rgba(78,110,93,0.12)',
  gold: '#97742C',
...
export const Chip = ({children, tone = 'white', style}: {children: ReactNode; tone?: 'white' | 'ink' | 'indigo' | 'rouge' | 'gold' | 'night'; style?: CSSProperties}) => {
...
export const LabelBlock = ({children, color = C.indigo, ink = false, size = 28, style}: {children: ReactNode; color?: string; ink?: boolean; size?: number; style?: CSSProperties}) => (
...
export const Neg = ({children, size = 24, style}: {children: ReactNode; size?: number; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, color: C.rouge, fontSize: size, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>
    <span style={{display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: size + 6, height: size + 6, borderRadius: 999, border: `3px solid ${C.rouge}`, color: C.rouge, fontSize: size - 4, fontWeight: 950, flexShrink: 0}}>×</span>
    <span>{children}</span>
  </span>
);

export const WaxStamp = ({children, delay = 0, tone = 'rouge'}: {children: ReactNode; delay?: number; tone?: 'rouge' | 'indigo' | 'gold'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const color = tone === 'rouge' ? C.rouge : tone === 'indigo' ? C.indigo : C.gold;
  return <span style={{display: 'inline-flex', padding: '3px 12px', border: `3px double ${color}`, color, fontSize: 22, fontWeight: 950, rotate: '-2deg', opacity: p, scale: 0.9 + p * 0.1, whiteSpace: 'nowrap', backgroundColor: 'rgba(250,246,234,0.65)'}}>{children}</span>;
};
...
export const Mark = ({children, color = C.indigo}: {children: ReactNode; color?: string}) => (
...
export const IconChip = ({icon, tone, title, children}: {icon: ReactNode; tone: string; title: string; children: ReactNode}) => (
  <div style={{display: 'flex', alignItems: 'center', gap: 12, backgroundColor: C.scrollDeep, borderLeft: `6px solid ${tone}`, padding: '8px 12px'}}>
...
export const Shell = ({code, kicker, title, children}: {code: string; kicker: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.scroll, color: C.ink, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', left: 62, top: 44, padding: '10px 16px', backgroundColor: C.night, borderLeft: `8px solid ${C.rouge}`}}>
      <span style={{fontSize: 22, fontWeight: 950, color: C.white, letterSpacing: 2}}>第23讲 · {code}</span>
    </div>
    <div style={{position: 'absolute', left: 250, right: 72, top: 34, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 22, borderBottom: `3px solid ${C.ink}`, paddingBottom: 10}}>
      ...
      <span style={{fontSize: 20, fontWeight: 850, color: C.indigo, textAlign: 'right', whiteSpace: 'nowrap'}}>{kicker}</span>
    </div>
