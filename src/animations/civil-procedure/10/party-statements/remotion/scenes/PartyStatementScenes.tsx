...
const Notebook=({code,title,children}:{code:string;title:string;children:ReactNode})=><div style={{position:'absolute',inset:0,overflow:'hidden',backgroundColor:N.page,color:N.ink,fontFamily:'"Segoe UI", "Microsoft YaHei", sans-serif'}}>{[220,340,460,580,700,820,940].map(y=><div key={y} style={{position:'absolute',left:0,right:0,top:y,height:2,backgroundColor:N.line,opacity:.55}}/>)}<div style={{position:'absolute',left:130,top:0,bottom:0,width:4,backgroundColor:N.red,opacity:.55}}/><div style={{position:'absolute',left:174,top:50,fontSize:18,fontWeight:850,color:N.blue}}>COURT NOTE / {code}</div><h1 style={{position:'absolute',left:174,top:86,margin:0,fontSize:56,fontWeight:850}}>{title}</h1><div style={{position:'absolute',left:174,right:70,top:174,height:4,backgroundColor:N.ink}}/>{children}<div style={{position:'absolute',left:174,bottom:28,fontSize:17,fontWeight:750,color:N.blue}}>当事人陈述 · 证明与程序边界</div></div>;
