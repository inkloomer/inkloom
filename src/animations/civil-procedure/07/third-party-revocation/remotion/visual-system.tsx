...
export const baseTextStyle: CSSProperties = {color: PALETTE.ink, fontFamily: 'var(--inkloom-animation-body)', letterSpacing: 0};
...
export const PrintBackdrop = ({frame}: {readonly frame: number}) => {
  ...
  return <>
    ...
    <div style={{position: 'absolute', left: 96, top: 76, color: PALETTE.violet, fontFamily: 'var(--inkloom-animation-meta)', fontSize: 18, letterSpacing: 2}}>PRINT SHOP / REMEDY 07</div>
    <div style={{position: 'absolute', right: 96, top: 76, color: PALETTE.vermilion, fontFamily: 'var(--inkloom-animation-meta)', fontSize: 17}}>PLATE REGISTERED</div>
  </>;
};
...
export const PrintHeading = ({index, eyebrow, title, accent, placement = 'top-left'}: {readonly index: string; readonly eyebrow: string; readonly title: string; readonly accent: Accent; readonly placement?: 'left-rail' | 'right-rail' | 'top-left' | 'top-right'}) => {
  if (placement === 'left-rail' || placement === 'right-rail') { const right = placement === 'right-rail'; return <MaskedReveal delay={4} duration={24} style={{position: 'absolute', left: right ? undefined : 58, right: right ? 58 : undefined, top: 250}}><div style={{width: 98, height: 545, boxSizing: 'border-box', padding: '18px 14px', border: `3px solid ${accentColor(accent)}`, backgroundColor: PALETTE.sheet}}><div style={{color: accentColor(accent), fontFamily: 'var(--inkloom-animation-mono)', fontSize: 24, fontWeight: 900, textAlign: 'center'}}>{index}</div><div style={{margin: '18px auto', width: 36, height: 3, backgroundColor: accentColor(accent)}} /><div style={{color: PALETTE.muted, fontFamily: 'var(--inkloom-animation-meta)', fontSize: 14, writingMode: 'vertical-rl'}}>{eyebrow.toUpperCase()}</div><div style={{margin: '22px auto 0', color: PALETTE.ink, fontSize: 35, fontWeight: 900, lineHeight: 1.12, writingMode: 'vertical-rl'}}>{title}</div></div></MaskedReveal>; }
  const right = placement === 'top-right';
  return <MaskedReveal delay={4} duration={24} style={{position: 'absolute', left: right ? undefined : 108, right: right ? 108 : undefined, top: 116}}><div style={{display: 'flex', flexDirection: right ? 'row-reverse' : 'row', alignItems: 'center', gap: 20, textAlign: right ? 'right' : 'left'}}><div style={{display: 'grid', width: 70, height: 70, placeItems: 'center', border: `3px solid ${accentColor(accent)}`, color: accentColor(accent), fontFamily: 'var(--inkloom-animation-mono)', fontSize: 23, fontWeight: 900}}>{index}</div><div><div style={{color: accentColor(accent), fontFamily: 'var(--inkloom-animation-mono)', fontSize: 17, letterSpacing: 2}}>{eyebrow.toUpperCase()}</div><div style={{marginTop: 7, color: PALETTE.ink, fontSize: 52, fontWeight: 900, lineHeight: 1}}>{title}</div></div></div></MaskedReveal>;
};
...
export const Plate = ({icon: Icon, label, detail, accent, left, top, opacity = 1, rotate = '0deg', flow = false}: {readonly icon: LucideIcon; readonly label: string; readonly detail?: string; readonly accent: Accent; readonly left: number; readonly top: number; readonly opacity?: number; readonly rotate?: string; readonly flow?: boolean}) => (
  <div style={{position: flow ? 'relative' : 'absolute', left: flow ? undefined : left, top: flow ? undefined : top, width: 430, minHeight: 220, boxSizing: 'border-box', padding: '28px 30px', opacity, rotate, backgroundColor: PALETTE.sheet, border: `2px solid ${accentColor(accent)}`, boxShadow: '8px 10px 0 rgba(42, 37, 31, 0.11)'}}>
    <div style={{display: 'flex', alignItems: 'center', gap: 14, color: accentColor(accent)}}><Icon size={34} strokeWidth={1.8} /><span style={{fontFamily: 'var(--inkloom-animation-meta)', fontSize: 15, letterSpacing: 1}}>PLATE / {label}</span></div>
...
export const PrintArrow = ({left, top, width, progress, accent, label}: {readonly left: number; readonly top: number; readonly width: number; readonly progress: number; readonly accent: Accent; readonly label?: string}) => <div style={{position: 'absolute', left, top, width, height: 64}}>{label ? <div style={{position: 'absolute', left: 0, top: -32, color: accentColor(accent), fontFamily: 'var(--inkloom-animation-meta)', fontSize: 22, fontWeight: 700, opacity: progress}}>{label}</div> : null}<div style={{position: 'absolute', left: 0, top: 28, width, height: 4, backgroundColor: accentColor(accent), scale: `${progress} 1`, transformOrigin: 'left center'}} /><ArrowRight size={34} strokeWidth={2} style={{position: 'absolute', right: -10, top: 11, color: accentColor(accent), opacity: interpolate(progress, [0.8, 1], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'})}} /></div>;
...
export const CropText = ({children, accent = 'violet'}: {readonly children: ReactNode; readonly accent?: Accent}) => <div style={{color: accentColor(accent), fontFamily: 'var(--inkloom-animation-mono)', fontSize: 16, letterSpacing: 1}}>{children}</div>;
