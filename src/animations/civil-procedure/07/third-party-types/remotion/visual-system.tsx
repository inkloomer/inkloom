...
export const RelationBackdrop = () => (
  <>
    ...
    <div style={{position: 'absolute', left: 76, top: 70, color: PALETTE.muted, fontFamily: MONO, fontSize: 18, letterSpacing: 2}}>CASE 07 / PARTY POSITIONING</div>
    <div style={{position: 'absolute', right: 76, top: 70, color: PALETTE.yellow, fontFamily: MONO, fontSize: 17}}>RELATION MAP / CIVIL PROCEDURE</div>
  </>
);
...
export const RelationHeading = ({index, eyebrow, title, accent, placement = 'top-left'}: {readonly index: string; readonly eyebrow: string; readonly title: string; readonly accent: Accent; readonly placement?: 'left-rail' | 'top-left' | 'top-right'}) => {
  if (placement === 'left-rail') return <MaskedReveal delay={4} duration={24} style={{position: 'absolute', left: 58, top: 250}}><div style={{width: 96, height: 540, boxSizing: 'border-box', padding: '18px 14px', border: `2px solid ${accentColor(accent)}`, backgroundColor: PALETTE.panel}}><div style={{color: accentColor(accent), fontFamily: MONO, fontSize: 25, fontWeight: 900, textAlign: 'center'}}>{index}</div><div style={{margin: '18px auto', width: 34, height: 2, backgroundColor: accentColor(accent)}} /><div style={{color: PALETTE.muted, fontFamily: MONO, fontSize: 14, textAlign: 'center', writingMode: 'vertical-rl'}}>{eyebrow.toUpperCase()}</div><div style={{...baseTextStyle, margin: '22px auto 0', fontSize: 35, fontWeight: 900, lineHeight: 1.12, writingMode: 'vertical-rl'}}>{title}</div></div></MaskedReveal>;
  const right = placement === 'top-right';
  return <MaskedReveal delay={4} duration={24} style={{position: 'absolute', left: right ? undefined : 94, right: right ? 94 : undefined, top: 108}}><div style={{display: 'flex', flexDirection: right ? 'row-reverse' : 'row', alignItems: 'center', gap: 22, textAlign: right ? 'right' : 'left'}}><div style={{display: 'grid', width: 74, height: 74, placeItems: 'center', border: `2px solid ${accentColor(accent)}`, color: accentColor(accent), fontFamily: MONO, fontSize: 25, fontWeight: 800}}>{index}</div><div><div style={{...baseTextStyle, color: accentColor(accent), fontFamily: MONO, fontSize: 17, letterSpacing: 2}}>{eyebrow.toUpperCase()}</div><div style={{...baseTextStyle, marginTop: 8, fontSize: 52, fontWeight: 900, lineHeight: 1}}>{title}</div></div></div></MaskedReveal>;
};
...
export const Pin = ({left, top, color = PALETTE.coral, label, fontSize = 22}: {readonly left: number; readonly top: number; readonly color?: string; readonly label?: string; readonly fontSize?: number}) => (
  <div style={{position: 'absolute', left, top, display: 'flex', alignItems: 'center', gap: 8, color, fontFamily: MONO, fontSize}}><div style={{width: 14, height: 14, borderRadius: '50%', backgroundColor: color, boxShadow: `0 0 0 5px ${color}33`}} />{label ? <span>{label}</span> : null}</div>
);
...
export const RelationArrow = ({left, top, width, progress, accent, label}: {readonly left: number; readonly top: number; readonly width: number; readonly progress: number; readonly accent: Accent; readonly label?: string}) => (
  <div style={{position: 'absolute', left, top, width, height: 60}}>
    {label ? <div style={{position: 'absolute', left: 0, top: -31, color: accentColor(accent), fontFamily: MONO, fontSize: 22, opacity: progress}}>{label}</div> : null}
...
export const RelationAnchor = ({left, top, label, accent}: {readonly left: number; readonly top: number; readonly label: string; readonly accent: Accent}) => (
  <div style={{position: 'absolute', left, top, display: 'flex', alignItems: 'center', gap: 9, color: accentColor(accent), fontFamily: MONO, fontSize: 23, fontWeight: 700}}><MapPin size={25} strokeWidth={1.8} /><span>{label}</span></div>
);
