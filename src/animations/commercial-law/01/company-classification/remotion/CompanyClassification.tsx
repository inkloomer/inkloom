...
export const CompanyClassification01Scene=()=>{
  ...
  return <Shell code="01.2" title="分公司 与 子公司">
    <div data-layout="branch-lanes-1" data-visual-anchor="comparison-axis" data-visual-grammar="twin-branch-contrast,personality-split" data-text-treatments="label-block,external-negation,soft-highlight" data-focal-rule="company-classification-scene-01-rule" data-focal-channels="contrast,spatial,icon" style={{position:'absolute',inset:0}}>
      <Building2 size={260} color={COLORS.blue} strokeWidth={1.2} style={{position:'absolute',left:'50%',top:560,translate:'-50% 0',opacity:0.07,pointerEvents:'none'}}/>
      ...
      <div data-final-knowledge="company-classification-scene-01-branch" style={{position:'absolute',left:24,top:86,width:720,padding:26,background:COLORS.paper,borderTop:'10px solid '+COLORS.blue,boxShadow:'0 0 0 3px '+COLORS.ink,opacity:enter(26,52),translate:slide(26,52,'-26px 0px')}}>
        ...
        <div style={{fontSize:22,fontWeight:700,color:COLORS.gray,marginTop:14,lineHeight:1.4}}>分公司设立于总公司名下，<span style={{fontWeight:900,color:COLORS.orange,borderBottom:'3px solid '+COLORS.orange,paddingBottom:1}}>不具备独立的法人资格</span></div>
      </div>
      <div data-final-knowledge="company-classification-scene-01-subsidiary" style={{position:'absolute',right:24,top:86,width:720,padding:26,background:COLORS.paper,borderTop:'10px solid '+COLORS.orange,boxShadow:'0 0 0 3px '+COLORS.ink,opacity:enter(44,70),translate:slide(44,70,'26px 0px')}}>
        ...
        <div style={{fontSize:22,fontWeight:700,color:COLORS.gray,marginTop:14,lineHeight:1.4}}>子公司是母公司投资设立的<span style={{fontWeight:900,color:COLORS.blue,background:COLORS.blue+'16',padding:'2px 8px'}}>独立法人主体</span></div>
      </div>
      <div data-final-knowledge="company-classification-scene-01-duty-branch" style={{position:'absolute',left:24,top:430,width:720,padding:'18px 24px',background:COLORS.blue+'12',border:'3px solid '+COLORS.blue,opacity:enter(66,92),translate:interpolate(frame,[66,92],['0px 20px','0px 0px'],CLAMP),display:'flex',alignItems:'center',gap:16}}>
        <Link size={36} color={COLORS.blue}/>
        <div>
      ...
      <div data-final-knowledge="company-classification-scene-01-duty-subsidiary" style={{position:'absolute',right:24,top:430,width:720,padding:'18px 24px',background:COLORS.orange+'14',border:'3px solid '+COLORS.orange,opacity:enter(82,108),translate:interpolate(frame,[82,108],['0px 20px','0px 0px'],CLAMP),display:'flex',alignItems:'center',gap:16}}>
        <Unlink size={36} color={COLORS.orange}/>
        <div>
...
export const CompanyClassification02Scene=()=>{
  ...
  const ClauseRow=({icon,label,left,right,delay}:{readonly icon:ReactNode;readonly label:string;readonly left:ReactNode;readonly right:ReactNode;readonly delay:number})=>(
    <div style={{display:'grid',gridTemplateColumns:'230px 1fr 1fr',gap:0,border:'4px solid '+COLORS.ink,background:COLORS.paper}}>
      <div style={{padding:'18px',background:COLORS.ink,color:COLORS.paper,display:'grid',gridTemplateColumns:'42px 1fr',gap:12,alignItems:'center',opacity:enter(delay,delay+16),translate:interpolate(frame,[delay,delay+16],['-18px 0px','0px 0px'],CLAMP)}}>
        {icon}
        <div style={{fontSize:26,fontWeight:900}}>{label}</div>
      </div>
      <div style={{padding:'18px 24px',borderRight:'3px dashed '+COLORS.gray,opacity:enter(delay+10,delay+30),translate:interpolate(frame,[delay+10,delay+30],['0px 16px','0px 0px'],CLAMP)}}>
      ...
      <div style={{padding:'18px 24px',opacity:enter(delay+22,delay+42),translate:interpolate(frame,[delay+22,delay+42],['0px 16px','0px 0px'],CLAMP)}}>
  ...
  return <Shell code="01.2" title="营业执照 与 诉讼地位">
    <div data-layout="clause-duel-2" data-visual-anchor="role-pair" data-visual-grammar="clause-duel,shared-license" data-text-treatments="thin-underline,label-block,soft-highlight" data-focal-rule="company-classification-scene-02-rule" data-focal-channels="contrast,locator,annotation" style={{position:'absolute',inset:0}}>
      <FileCheck2 size={130} color={COLORS.blue} strokeWidth={1.2} style={{position:'absolute',right:40,top:622,opacity:0.08,pointerEvents:'none'}}/>
      ...
      <div data-final-knowledge="company-classification-scene-02-license-branch" style={{position:'absolute',left:0,right:0,top:66}}>
        <ClauseRow icon={<FileCheck2 size={38} color={COLORS.paper}/>} label="营业执照" delay={28} left={<span style={{fontWeight:900,borderBottom:'4px solid '+COLORS.blue,paddingBottom:2}}>需办理</span>} right={<span style={{fontWeight:900,borderBottom:'4px solid '+COLORS.orange,paddingBottom:2}}>需办理</span>}/>
      </div>
      <div data-final-knowledge="company-classification-scene-02-license-subsidiary" style={{position:'absolute',left:0,right:0,top:236,padding:'16px 22px',border:'3px solid '+COLORS.blue,background:COLORS.paper,fontSize:24,fontWeight:800,lineHeight:1.45,opacity:enter(52,78),display:'flex',alignItems:'center',gap:14}}><MapPin size={34} color={COLORS.blue}/><span>分公司<span style={{background:COLORS.blue+'22',padding:'2px 8px',fontWeight:900}}>还需在所在地申领</span>、<span style={{background:COLORS.orange+'20',padding:'2px 8px',fontWeight:900}}>总公司备案</span>；子公司仅需办理</span></div>
      <div data-final-knowledge="company-classification-scene-02-suit-branch" style={{position:'absolute',left:0,right:0,top:390}}>
        <ClauseRow icon={<Scale size={38} color={COLORS.paper}/>} label="诉讼" delay={66} left={<span>独立参诉，<span style={{fontWeight:900,background:COLORS.blue+'1E',padding:'2px 6px'}}>具备原被告资格</span>；责任由<span style={{fontWeight:900,color:COLORS.blue}}>总公司兜底</span></span>} right={<span>独立参诉，<span style={{fontWeight:900,background:COLORS.orange+'1C',padding:'2px 6px'}}>独立承担责任</span></span>}/>
      </div>
      <div data-final-knowledge="company-classification-scene-02-suit-subsidiary" style={{position:'absolute',left:0,right:0,top:560,padding:'16px 22px',border:'3px dashed '+COLORS.orange,background:COLORS.paper,fontSize:24,fontWeight:800,lineHeight:1.45,opacity:enter(82,108),display:'flex',alignItems:'center',gap:14}}><Gavel size={34} color={COLORS.orange}/><span>子公司诉讼独立、责任独立——<span style={{background:COLORS.orange+'20',padding:'2px 8px',fontWeight:900}}>总公司不兜底</span></span></div>
    </div>
  </Shell>;
};
...
export const CompanyClassification03Scene=()=>{
  ...
  const stepStyle=(color:string,delay:number):CSSProperties=>({display:'grid',gridTemplateColumns:'70px 1fr',gap:18,alignItems:'center',padding:'16px 22px',border:'4px solid '+COLORS.ink,background:COLORS.paper,opacity:enter(delay,delay+26),translate:interpolate(frame,[delay,delay+26],['0px 22px','0px 0px'],CLAMP)});
  return <Shell code="01.2" title="分公司的执行顺序">
    <div data-layout="execution-chain-3" data-visual-anchor="flow-path" data-visual-grammar="execution-cascade,branch-fallback" data-text-treatments="stamp,thin-underline,soft-highlight" data-focal-rule="company-classification-scene-03-rule" data-focal-channels="connector,motion,enclosure" style={{position:'absolute',inset:0}}>
      <ListOrdered size={260} color={COLORS.orange} strokeWidth={1.2} style={{position:'absolute',left:'50%',top:430,translate:'-50% 0',opacity:0.07,pointerEvents:'none'}}/>
      ...
      <div style={{position:'absolute',left:90,right:0,top:64,display:'grid',gap:16}}>
        <div data-final-knowledge="company-classification-scene-03-step-0" style={{...stepStyle(COLORS.blue,28),gridTemplateColumns:'70px 56px 1fr'}}>
          ...
          <div><div style={{fontSize:28,fontWeight:900,marginBottom:6}}>分公司财产</div><div style={{fontSize:22,fontWeight:700,color:COLORS.gray,lineHeight:1.4}}>先以<span style={{fontWeight:900,color:COLORS.blue,background:COLORS.blue+'16',padding:'1px 6px'}}>分公司财产</span>清偿债务</div></div>
        </div>
        <div data-final-knowledge="company-classification-scene-03-step-1" style={{...stepStyle(COLORS.ink,52),gridTemplateColumns:'70px 56px 1fr'}}>
          ...
          <div><div style={{fontSize:28,fontWeight:900,marginBottom:6}}>追加总公司为被执行人</div><div style={{fontSize:22,fontWeight:700,color:COLORS.gray,lineHeight:1.4}}>分公司财产<span style={{fontWeight:900,color:COLORS.orange,background:COLORS.orange+'16',padding:'1px 6px'}}>不足</span>时，可<span style={{fontWeight:900,color:COLORS.ink,borderBottom:'3px solid '+COLORS.orange,paddingBottom:1}}>追加总公司</span></div></div>
        </div>
        <div data-final-knowledge="company-classification-scene-03-step-2" style={{...stepStyle(COLORS.orange,76),gridTemplateColumns:'70px 56px 1fr'}}>
          ...
          <div><div style={{fontSize:28,fontWeight:900,marginBottom:6}}>直接执行其他分公司财产</div><div style={{fontSize:22,fontWeight:700,color:COLORS.gray,lineHeight:1.4}}>总公司财产<span style={{fontWeight:900,color:COLORS.blue,background:COLORS.blue+'14',padding:'1px 6px'}}>仍不能清偿</span>的，直接执行<span style={{fontWeight:900,color:COLORS.orange}}>其他分公司财产</span></div></div>
        </div>
      </div>
