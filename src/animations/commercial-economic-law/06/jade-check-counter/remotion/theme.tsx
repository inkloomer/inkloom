...
export const C = {
  ...
  pine: '#1F4A38',
  ink: '#12241C',
  inkDim: 'rgba(18,36,28,0.62)',
  copper: '#A8663A',
  copperSoft: 'rgba(168,102,58,0.16)',
  crimson: '#B0392C',
  crimsonDark: '#822D20',
  crimsonLight: '#FFA793',
  gold: '#B8912E',
  shadow: 'rgba(10,25,18,0.25)',
  panel: 'rgba(255,255,255,0.72)',
  panelLine: 'rgba(18,36,28,0.26)',
} as const;
...
export const Chip = ({children, tone = 'paper', style}: {children: ReactNode; tone?: 'paper' | 'mist' | 'pine' | 'crimson' | 'copper' | 'ink' | 'panel'; style?: CSSProperties}) => {
  const bg = tone === 'paper' ? C.paper : tone === 'panel' ? C.panel : C[tone as 'mist' | 'pine' | 'crimson' | 'copper' | 'ink'];
  const color = tone === 'pine' || tone === 'crimson' || tone === 'copper' || tone === 'ink' ? C.paper : C.ink;
...
export const SoftHi = ({children, light = false, style}: {children: ReactNode; light?: boolean; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, backgroundColor: C.copperSoft, padding: '4px 12px', borderRadius: 8, color: light ? C.paper : C.ink, fontSize: 24, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>{children}</span>
);
...
export const Neg = ({children, light = false, size = 26}: {children: ReactNode; light?: boolean; size?: number}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, color: light ? C.crimsonLight : C.crimsonDark, fontSize: size, fontWeight: 900, whiteSpace: 'nowrap'}}>
    <span style={{display: 'inline-flex', fontSize: size + 4, fontWeight: 950}}>×</span>
    <span>{children}</span>
  </span>
);

export const Stamp = ({children, delay = 0, tone = 'copper'}: {children: ReactNode; delay?: number; tone?: 'copper' | 'pine' | 'crimson' | 'paper'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const border = tone === 'copper' ? C.copper : tone === 'pine' ? C.pine : tone === 'crimson' ? C.crimson : C.gold;
...
export const Shell = ({code, title, children}: {code: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.field, color: C.ink, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', inset: 0, backgroundImage: 'repeating-linear-gradient(0deg, rgba(18,36,28,0.04) 0 2px, transparent 2px 180px)'}} />
    <div style={{position: 'absolute', left: 72, top: 42, fontSize: 17, fontWeight: 900, letterSpacing: 3, color: C.copper}}>COMMERCIAL LAW · JADE CHECK COUNTER / {code}</div>
    ...
    <div style={{position: 'absolute', left: 72, right: 72, bottom: 40, display: 'flex', justifyContent: 'space-between', fontSize: 16, fontWeight: 800, color: C.inkDim}}>
      <span>商经知 · 真金题 考点5 支票</span>
      <span>2021金题 · 2023金题</span>
    </div>
  </AbsoluteFill>
);
