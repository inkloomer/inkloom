...
export const C = {
  ...
  trunkLine: '#6B4536',
  lacquer: '#B4372A',
  lacquerSoft: 'rgba(180,55,42,0.30)',
  gold: '#D9A94E',
  goldSoft: 'rgba(217,169,78,0.26)',
  ivory: '#F4EDDD',
  ivoryDim: 'rgba(244,237,221,0.62)',
  ink: '#241A12',
  curtain: '#31424E',
  curtainSoft: 'rgba(49,66,78,0.55)',
  jade: '#4E7C5B',
  danger: '#8C2F1F',
  negLight: '#FFB4A0',
  shadow: 'rgba(12,6,4,0.5)',
  panel: 'rgba(244,237,221,0.07)',
  panelLine: 'rgba(244,237,221,0.30)',
} as const;
...
export const Chip = ({children, tone = 'ivory', style}: {children: ReactNode; tone?: 'ivory' | 'gold' | 'trunk' | 'lacquer' | 'jade' | 'ink' | 'panel' | 'curtain'; style?: CSSProperties}) => {
  const bg = tone === 'ivory' ? C.ivory : tone === 'panel' ? C.panel : C[tone as 'gold' | 'trunk' | 'lacquer' | 'jade' | 'ink' | 'curtain'];
...
export const Stamp = ({children, delay = 0, tone = 'gold'}: {children: ReactNode; delay?: number; tone?: 'gold' | 'jade' | 'lacquer' | 'ivory'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const border = tone === 'gold' ? C.gold : tone === 'jade' ? C.jade : tone === 'lacquer' ? C.lacquer : C.ivory;
...
export const Shell = ({code, title, children}: {code: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.field, color: C.ivory, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', inset: 0, backgroundImage: 'repeating-linear-gradient(0deg, rgba(244,237,221,0.04) 0 2px, transparent 2px 178px)'}} />
    <div style={{position: 'absolute', left: 72, top: 42, fontSize: 17, fontWeight: 900, letterSpacing: 3, color: C.gold}}>COMMERCIAL LAW · NOMINEE & MASKS / {code}</div>
    ...
    <div style={{position: 'absolute', left: 72, right: 72, bottom: 40, display: 'flex', justifyContent: 'space-between', fontSize: 16, fontWeight: 800, color: C.ivoryDim}}>
      <span>商经知 · 真金题 考点6 名义股东与实际出资人</span>
      <span>WHO WEARS THE MASK OWNS THE SEAT</span>
    </div>
  </AbsoluteFill>
);
