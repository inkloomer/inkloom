...
export const C = {
  ...
  plankLine: '#52442F',
  lantern: '#E8A23C',
  lanternSoft: 'rgba(232,162,60,0.26)',
  paper: '#F2E7CE',
  paperDim: 'rgba(242,231,206,0.62)',
  ink: '#1F180E',
  seal: '#B23A2E',
  ...
  panel: 'rgba(242,231,206,0.07)',
  panelLine: 'rgba(242,231,206,0.30)',
} as const;
...
export const Chip = ({children, tone = 'paper', style}: {children: ReactNode; tone?: 'paper' | 'lantern' | 'plank' | 'seal' | 'jade' | 'ink' | 'panel'; style?: CSSProperties}) => {
  const bg = tone === 'paper' ? C.paper : tone === 'panel' ? C.panel : C[tone as 'lantern' | 'plank' | 'seal' | 'jade' | 'ink'];
  const color = tone === 'paper' || tone === 'panel' || tone === 'lantern' ? C.ink : C.paper;
...
export const LabelBlock = ({children, color = C.lantern, light = false, size = 28, style}: {children: ReactNode; color?: string; light?: boolean; size?: number; style?: CSSProperties}) => (
...
export const SoftHi = ({children, dark = true, style}: {children: ReactNode; dark?: boolean; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, backgroundColor: C.lanternSoft, padding: '4px 12px', borderRadius: 8, color: dark ? C.paper : C.ink, fontSize: 24, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>{children}</span>
);
...
export const Stamp = ({children, delay = 0, tone = 'lantern'}: {children: ReactNode; delay?: number; tone?: 'lantern' | 'jade' | 'seal' | 'paper'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const border = tone === 'lantern' ? C.lantern : tone === 'jade' ? C.jade : tone === 'seal' ? C.seal : C.paper;
...
export const Shell = ({code, title, children}: {code: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.field, color: C.paper, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', inset: 0, backgroundImage: 'repeating-linear-gradient(0deg, rgba(242,231,206,0.035) 0 2px, transparent 2px 180px)'}} />
    <div style={{position: 'absolute', left: 72, top: 42, fontSize: 17, fontWeight: 900, letterSpacing: 3, color: C.lantern}}>COMMERCIAL LAW · SHOP CLOSING HALL / {code}</div>
    <div style={{position: 'absolute', left: 72, top: 70, fontSize: 46, fontWeight: 950, color: C.paper}}>{title}</div>
    <div style={{position: 'absolute', left: 72, right: 72, top: 138, height: 5, backgroundColor: C.lantern}} />
    ...
    <div style={{position: 'absolute', left: 72, right: 72, bottom: 40, display: 'flex', justifyContent: 'space-between', fontSize: 16, fontWeight: 800, color: C.paperDim}}>
      <span>商经知 · 真金题 考点13 公司的消亡</span>
      <span>CLOSE THE DOOR — COUNT — TAKE DOWN THE SIGN</span>
    </div>
  </AbsoluteFill>
);
