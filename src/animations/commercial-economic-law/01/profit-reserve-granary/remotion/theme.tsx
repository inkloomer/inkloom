...
export const C = {
  ...
  woodLine: '#A9763F',
  husk: '#D9B36C',
  huskSoft: 'rgba(217,179,108,0.42)',
  teal: '#2E6E63',
  tealSoft: 'rgba(46,110,99,0.16)',
  seal: '#B03A2E',
  jade: '#3F7D4E',
  paper: '#FBF6E8',
  paperDim: '#E3D7B8',
  danger: '#8C2F1F',
  negLight: '#FFB4A0',
  shadow: 'rgba(51,41,26,0.30)',
  panel: 'rgba(51,41,26,0.05)',
  panelLine: 'rgba(51,41,26,0.28)',
} as const;
...
export const Chip = ({children, tone = 'rice', style}: {children: ReactNode; tone?: 'rice' | 'wood' | 'teal' | 'seal' | 'jade' | 'ink' | 'panel'; style?: CSSProperties}) => {
  const bg = tone === 'rice' ? C.paper : tone === 'panel' ? C.panel : C[tone as 'wood' | 'teal' | 'seal' | 'jade' | 'ink'];
  const color = tone === 'rice' || tone === 'panel' ? C.ink : C.paper;
...
export const SoftHi = ({children, dark = false, style}: {children: ReactNode; dark?: boolean; style?: CSSProperties}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, backgroundColor: dark ? 'rgba(217,179,108,0.35)' : C.huskSoft, padding: '4px 12px', borderRadius: 8, color: dark ? C.paper : C.ink, fontSize: 24, fontWeight: 900, whiteSpace: 'nowrap', ...style}}>{children}</span>
);
...
export const Neg = ({children, dark = false, size = 26}: {children: ReactNode; dark?: boolean; size?: number}) => (
  <span style={{display: 'inline-flex', alignItems: 'center', gap: 8, color: dark ? C.negLight : C.danger, fontSize: size, fontWeight: 900, whiteSpace: 'nowrap'}}>
    <span style={{display: 'inline-flex', fontSize: size + 4, fontWeight: 950}}>×</span>
    <span>{children}</span>
  </span>
);

export const Stamp = ({children, delay = 0, tone = 'seal'}: {children: ReactNode; delay?: number; tone?: 'seal' | 'jade' | 'teal' | 'wood' | 'rice'}) => {
  const frame = useCurrentFrame();
  const p = reveal(frame, delay);
  const border = tone === 'seal' ? C.seal : tone === 'jade' ? C.jade : tone === 'teal' ? C.teal : tone === 'wood' ? C.wood : C.ink;
  const bg = tone === 'rice' ? C.paper : 'transparent';
...
export const Shell = ({code, title, children}: {code: string; title: string; children: ReactNode}) => (
  <AbsoluteFill data-player-control-safe-bottom={PLAYER_CONTROL_SAFE_BOTTOM} style={{overflow: 'hidden', backgroundColor: C.field, color: C.ink, fontFamily: 'var(--inkloom-animation-body)'}}>
    <div style={{position: 'absolute', inset: 0, backgroundImage: 'repeating-linear-gradient(90deg, rgba(51,41,26,0.05) 0 3px, transparent 3px 190px)'}} />
    <div style={{position: 'absolute', left: 72, top: 42, fontSize: 17, fontWeight: 900, letterSpacing: 3, color: C.teal}}>COMMERCIAL LAW · PROFIT RESERVE / {code}</div>
    ...
    <div style={{position: 'absolute', left: 72, right: 72, bottom: 40, display: 'flex', justifyContent: 'space-between', fontSize: 16, fontWeight: 800, color: C.ink, opacity: 0.62}}>
      <span>商经知 · 真金题 考点3 公司的财务会计制度</span>
      <span>FILL — RESERVE — SHARE</span>
    </div>
  </AbsoluteFill>
);
