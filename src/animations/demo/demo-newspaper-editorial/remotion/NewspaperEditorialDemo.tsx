...
export const NewspaperEditorialDemo = () => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#f3f1ea', overflow: 'hidden', color: '#111111', fontFamily: 'var(--inkloom-animation-body)'}}>
      <div style={{position: 'absolute', left: 82, right: 82, top: 72, height: 8, backgroundColor: '#111111'}} />
      <div style={{position: 'absolute', left: 82, top: 95, fontFamily: 'var(--inkloom-animation-body)', fontSize: 22, fontWeight: 700}}>THE RULE REVIEW</div>
      <div style={{position: 'absolute', right: 82, top: 98, fontFamily: 'var(--inkloom-animation-meta)', fontSize: 18}}>NO. 010 / DEMO EDITION</div>
      <Interactive.Div
        name="Editorial headline"
        style={{
          position: 'absolute', left: 82, top: 150, width: 1480, fontFamily: 'var(--inkloom-animation-body)',
      ...
      {[
        ...
        >
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 24}}>
            <span style={{fontFamily: 'var(--inkloom-animation-body)', fontSize: 66}}>{title}</span><Icon size={52} strokeWidth={1.7} />
          </div>
          <div style={{marginTop: 30, fontSize: 38, fontWeight: 900}}>{zh}</div>
          <div style={{marginTop: 30, columnCount: width > 600 ? 2 : 1, columnGap: 30, fontFamily: 'var(--inkloom-animation-body)', fontSize: 24, lineHeight: 1.65}}>
            请求权基础　构成要件　法律效果　把判断拆成可检验的步骤，避免从材料直接跳到答案。
          </div>
        </Interactive.Div>
      ))}
      <div style={{position: 'absolute', left: 82, right: 82, bottom: 66, display: 'flex', justifyContent: 'space-between', borderTop: '2px solid #111111', paddingTop: 16, fontFamily: 'var(--inkloom-animation-meta)', fontSize: 18}}>
